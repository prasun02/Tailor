import { Archive, RotateCcw, ShieldAlert, SlidersHorizontal } from 'lucide-react';
import { useState, type ReactNode } from 'react';
import { EmptyState } from '../components/ui/EmptyState';
import { Loading } from '../components/ui/Loading';
import { StyleFieldForm } from '../features/measurements/components/ConfigurationForms';
import { styleFieldToFormValues } from '../features/measurements/configurationFormMappers';
import { getStyleOptions } from '../features/measurements/dynamicValidation';
import {
  useArchiveStyleField,
  useCreateStyleField,
  useGarmentTypes,
  useRestoreStyleField,
  useStyleFields,
  useUpdateStyleField,
} from '../features/measurements/configurationHooks';
import type { StyleFieldFormValues } from '../features/measurements/configurationSchemas';
import type { StyleField } from '../features/measurements/types';
import { useShop } from '../features/shop/shopContext';
import { canManageConfiguration } from '../utils/authorization';
import { cn } from '../utils/cn';

export function StyleFieldsSettingsPage() {
  ...
  const fieldsQuery = useStyleFields(currentShopId, garmentTypeId, includeArchived);
  const createField = useCreateStyleField(currentShopId ?? '');
  const updateField = useUpdateStyleField(currentShopId ?? '', editing?.id ?? '');
  const archiveField = useArchiveStyleField(currentShopId ?? '');
  const restoreField = useRestoreStyleField(currentShopId ?? '');
  const garments = garmentsQuery.data ?? [];

  if (!canManage) {
    return <EmptyState icon={ShieldAlert} title="Settings are restricted" message="Only owners and managers can manage style fields." />;
  }

  async function submitField(values: StyleFieldFormValues) {
    if (editing) {
  ...
  return (
    <FieldSettingsLayout icon={SlidersHorizontal} title="Style fields" description="Configure order-time style preferences. These are kept separate from body measurements.">
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(18rem,24rem)]">
        <section className="space-y-3 rounded-lg border border-slate-200 bg-white p-4 shadow-panel">
          ...
          {fieldsQuery.isLoading ? <Loading label="Loading style fields" /> : null}
          ...
          {fieldsQuery.data?.map((field) => (
            <FieldRow
              ...
              meta={`${field.field_type} - sort ${field.sort_order}`}
          ...
          {fieldsQuery.data?.length === 0 ? <EmptyState icon={SlidersHorizontal} title="No style fields configured" message="Add style preference fields for orders." /> : null}
        </section>
        <StyleFieldForm
          key={editing?.id ?? 'new'}
          garments={garments}
          initialValues={editing ? styleFieldToFormValues(editing) : undefined}
          submitLabel={editing ? 'Save style field' : 'Add style field'}
...
function FieldSettingsLayout({ icon: Icon, title, description, children }: { icon: typeof SlidersHorizontal; title: string; description: string; children: ReactNode }) {
  return (
...
function FieldRow({ label, fieldKey, meta, options, archived, onEdit, onArchive, onRestore }: { label: string; fieldKey: string; meta: string; options: string[]; archived: boolean; onEdit: () => void; onArchive: () => void; onRestore: () => void }) {
  return (
    <div className="rounded-lg border border-slate-200 p-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          {options.length > 0 ? <p className="mt-2 text-sm text-slate-500">Options: {options.join(', ')}</p> : null}
        </div>
